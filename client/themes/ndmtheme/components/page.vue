<template lang="pug">
  v-app(v-scroll='upBtnScroll', :dark='$vuetify.theme.dark', :class='$vuetify.rtl ? `is-rtl` : `is-ltr`')
    ...
    v-navigation-drawer(
      v-if='navMode !== `NONE`'
      :class='$vuetify.theme.dark ? `grey darken-4-d4` : `deep-purple`'
      dark
      app
      clipped
      ...
      vue-scroll(:ops='scrollStyle')
        nav-sidebar(:color='$vuetify.theme.dark ? `grey darken-4-d4` : `deep-purple`', :items='sidebarDecoded', :nav-mode='navMode')
        ...
        v-layout(row)
          v-flex.page-col-sd(lg3, xl2, v-if='$vuetify.breakpoint.lgAndUp')
            v-card.mb-5(v-if='tocDecoded.length')
              .overline.pa-5.pb-0(:class='$vuetify.theme.dark ? `blue--text text--lighten-2` : `deep-purple--text`') {{$t('common:page.toc')}}
